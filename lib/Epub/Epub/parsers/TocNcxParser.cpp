...
bool TocNcxParser::setup() {
  ...
  if (!parser) {
    LOG("TOC", "Couldn't allocate memory for parser");
    return false;
  }
...
size_t TocNcxParser::write(const uint8_t* buffer, const size_t size) {
  ...
  while (remainingInBuffer > 0) {
    ...
    if (!buf) {
      LOG("TOC", "Couldn't allocate memory for buffer");
      XML_StopParser(parser, XML_FALSE);                // Stop any pending processing
      XML_SetElementHandler(parser, nullptr, nullptr);  // Clear callbacks
      XML_SetCharacterDataHandler(parser, nullptr);
      XML_ParserFree(parser);
    ...
    if (XML_ParseBuffer(parser, static_cast<int>(toRead), remainingSize == toRead) == XML_STATUS_ERROR) {
      LOG("TOC", "Parse error at line %lu: %s", XML_GetCurrentLineNumber(parser),
