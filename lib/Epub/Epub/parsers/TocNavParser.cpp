...
bool TocNavParser::setup() {
  ...
  if (!parser) {
    LOG("NAV", "Couldn't allocate memory for parser");
    return false;
  }
...
size_t TocNavParser::write(const uint8_t* buffer, const size_t size) {
  ...
  while (remainingInBuffer > 0) {
    ...
    if (!buf) {
      LOG("NAV", "Couldn't allocate memory for buffer");
      XML_StopParser(parser, XML_FALSE);
      XML_SetElementHandler(parser, nullptr, nullptr);
      XML_SetCharacterDataHandler(parser, nullptr);
      XML_ParserFree(parser);
    ...
    if (XML_ParseBuffer(parser, static_cast<int>(toRead), remainingSize == toRead) == XML_STATUS_ERROR) {
      LOG("NAV", "Parse error at line %lu: %s", XML_GetCurrentLineNumber(parser),
...
void XMLCALL TocNavParser::endElement(void* userData, const XML_Char* name) {
  ...
  if (strcmp(name, "nav") == 0 && self->state >= IN_NAV_TOC) {
    self->state = IN_BODY;
    LOG("NAV", "Finished parsing nav toc");
    return;
  }
}
