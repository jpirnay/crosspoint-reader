...
XtcError XtcParser::open(const char* filepath) {
  ...
  if (m_lastError != XtcError::OK) {
    LOG("XTC", "Failed to read header: %s", errorToString(m_lastError));
    m_file.close();
    return m_lastError;
  }
  ...
  if (m_header.hasMetadata) {
    ...
    if (m_lastError != XtcError::OK) {
      LOG("XTC", "Failed to read title: %s", errorToString(m_lastError));
      m_file.close();
      return m_lastError;
    }
    m_lastError = readAuthor();
    if (m_lastError != XtcError::OK) {
      LOG("XTC", "Failed to read author: %s", errorToString(m_lastError));
      m_file.close();
      return m_lastError;
    }
  }
  ...
  if (m_lastError != XtcError::OK) {
    LOG("XTC", "Failed to read page table: %s", errorToString(m_lastError));
    m_file.close();
    return m_lastError;
  }
  ...
  if (m_lastError != XtcError::OK) {
    LOG("XTC", "Failed to read chapters: %s", errorToString(m_lastError));
    m_file.close();
    return m_lastError;
  }

  m_isOpen = true;
  LOG("XTC", "Opened file: %s (%u pages, %dx%d)", filepath, m_header.pageCount, m_defaultWidth, m_defaultHeight);
  return XtcError::OK;
}
...
XtcError XtcParser::readHeader() {
  ...
  if (m_header.magic != XTC_MAGIC && m_header.magic != XTCH_MAGIC) {
    LOG("XTC", "Invalid magic: 0x%08X (expected 0x%08X or 0x%08X)", m_header.magic, XTC_MAGIC, XTCH_MAGIC);
  ...
  if (!validVersion) {
    LOG("XTC", "Unsupported version: %u.%u", m_header.versionMajor, m_header.versionMinor);
  ...
  LOG("XTC", "Header: magic=0x%08X (%s), ver=%u.%u, pages=%u, bitDepth=%u", m_header.magic,
      (m_header.magic == XTCH_MAGIC) ? "XTCH" : "XTC", m_header.versionMajor, m_header.versionMinor, m_header.pageCount,
      m_bitDepth);
...
XtcError XtcParser::readAuthor() {
  ...
  LOG("XTC", "Author: %s", m_author.c_str());
  return XtcError::OK;
}

XtcError XtcParser::readPageTable() {
  if (m_header.pageTableOffset == 0) {
    LOG("XTC", "Page table offset is 0, cannot read");
    return XtcError::CORRUPTED_HEADER;
  }

  // Seek to page table
  if (!m_file.seek(m_header.pageTableOffset)) {
    LOG("XTC", "Failed to seek to page table at %llu", m_header.pageTableOffset);
    return XtcError::READ_ERROR;
  }
  ...
  for (uint16_t i = 0; i < m_header.pageCount; i++) {
    ...
    if (bytesRead != sizeof(PageTableEntry)) {
      LOG("XTC", "Failed to read page table entry %u", i);
      return XtcError::READ_ERROR;
    }
  ...
  LOG("XTC", "Read %u page table entries", m_header.pageCount);
  return XtcError::OK;
}
...
XtcError XtcParser::readChapters() {
  ...
  LOG("XTC", "Chapters: %u", static_cast<unsigned int>(m_chapters.size()));
  return XtcError::OK;
}
...
size_t XtcParser::loadPage(uint32_t pageIndex, uint8_t* buffer, size_t bufferSize) {
  ...
  if (!m_file.seek(page.offset)) {
    LOG("XTC", "Failed to seek to page %u at offset %lu", pageIndex, page.offset);
  ...
  if (headerRead != sizeof(XtgPageHeader)) {
    LOG("XTC", "Failed to read page header for page %u", pageIndex);
    m_lastError = XtcError::READ_ERROR;
    return 0;
  }
  ...
  if (pageHeader.magic != expectedMagic) {
    LOG("XTC", "Invalid page magic for page %u: 0x%08X (expected 0x%08X)", pageIndex, pageHeader.magic, expectedMagic);
  ...
  if (bufferSize < bitmapSize) {
    LOG("XTC", "Buffer too small: need %u, have %u", bitmapSize, bufferSize);
  ...
  if (bytesRead != bitmapSize) {
    LOG("XTC", "Page read error: expected %u, got %u", bitmapSize, bytesRead);
    m_lastError = XtcError::READ_ERROR;
    return 0;
  }
