...
void FontDecompressor::clearGlyphCache() {
...
bool FontDecompressor::decompressGroup(const EpdFontData* fontData, uint16_t groupIndex, uint8_t* outBuf,
  ...
  if (fontData->bitmapSource) {
    ...
    if (!fontData->bitmapSource->read(group.compressedOffset, _compressedStagingBuf, group.compressedSize)) {
      ...
      LOG_ERR("FDC", "IBitmapSource read failed for group %u", groupIndex);
      return false;
    }
    inflateReader.setSource(_compressedStagingBuf, group.compressedSize);
  } else {
    // Built-in font: direct pointer into flash rodata
...
const uint8_t* FontDecompressor::getBitmap(const EpdFontData* fontData, const EpdGlyph* glyph, uint32_t glyphIndex) {
  ...
  // Check per-glyph LRU cache (custom/SPIFFS fonts: misses are more frequent since
  // SPIFFS reads are slow and prewarm may not cover all repeated glyphs across pages).
  if (fontData->bitmapSource) {
  ...
  // Fallback: glyph wasn't in the page buffer or glyph cache — decompress its group transiently.
  ...
  // Store in per-glyph cache for custom/SPIFFS fonts so repeated misses avoid re-decompressing.
  if (fontData->bitmapSource) glyphCacheStore(fontData, glyphIndex, _hotGlyphBuf);
...
int FontDecompressor::prewarmCache(const EpdFontData* fontData, const char* utf8Text) {
  ...
  // One malloc/free per group per prewarm call. Only one group buffer and the page buffer coexist
  // at a time — peak heap = page buffer + largest single group in this font.
