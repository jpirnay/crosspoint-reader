...
class FontDecompressor {
 public:
  ...
  // Free all cached data (page buffers and glyph LRU cache).
  void clearCache();
 ...
 private:
  ...
  // Compressed staging buffer: IBitmapSource reads into this before inflation.
  // Sized to the largest compressed group across all built-in fonts (bookerly_18: 16 KB).
  // Custom fonts must stay within this limit (enforced by fontconvert.py --binary).
  ...
  // Also acts as the source when storing into the per-glyph LRU cache.
