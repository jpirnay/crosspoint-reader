...
bool inflateOneShot(const uint8_t* inputBuf, const size_t deflatedSize, uint8_t* outputBuf, const size_t inflatedSize) {
  ...
  if (!inflator) {
    LOG("ZIP", "Failed to allocate memory for inflator");
    return false;
  }
  ...
  if (status != TINFL_STATUS_DONE) {
    LOG("ZIP", "tinfl_decompress() failed with status %d", status);
    return false;
  }
...
long ZipFile::getDataOffset(const FileStatSlim& fileStat) {
  ...
  if (read != localHeaderSize) {
    LOG("ZIP", "Something went wrong reading the local header");
    return -1;
  }

  if (pLocalHeader[0] + (pLocalHeader[1] << 8) + (pLocalHeader[2] << 16) + (pLocalHeader[3] << 24) !=
      0x04034b50 /* MZ_ZIP_LOCAL_DIR_HEADER_SIG */) {
    LOG("ZIP", "Not a valid zip file header");
    return -1;
  }
...
bool ZipFile::loadZipDetails() {
  ...
  if (fileSize < 22) {
    LOG("ZIP", "File too small to be a valid zip");
    if (!wasOpen) {
      close();
    }
  ...
  if (!buffer) {
    LOG("ZIP", "Failed to allocate memory for EOCD scan buffer");
    if (!wasOpen) {
      close();
    }
    return false;
  ...
  if (foundOffset == -1) {
    LOG("ZIP", "EOCD signature not found in zip file");
    free(buffer);
    if (!wasOpen) {
      close();
    }
...
uint8_t* ZipFile::readFileToMemory(const char* filename, size_t* size, const bool trailingNullByte) {
  ...
  if (data == nullptr) {
    LOG("ZIP", "Failed to allocate memory for output buffer (%zu bytes)", dataSize);
    if (!wasOpen) {
      close();
    }
    return nullptr;
  ...
  if (fileStat.method == MZ_NO_COMPRESSION) {
    ...
    if (deflatedData == nullptr) {
      LOG("ZIP", "Failed to allocate memory for decompression buffer");
      if (!wasOpen) {
        close();
      }
      return nullptr;
    ...
    if (dataRead != deflatedDataSize) {
      LOG("ZIP", "Failed to read data, expected %d got %d", deflatedDataSize, dataRead);
      free(deflatedData);
      free(data);
      return nullptr;
    }
    ...
    if (!success) {
      LOG("ZIP", "Failed to inflate file");
      free(data);
      return nullptr;
    }
    ...
    LOG("ZIP", "Unsupported compression method");
    if (!wasOpen) {
      close();
    }
    return nullptr;
...
bool ZipFile::readFileToStream(const char* filename, Print& out, const size_t chunkSize) {
  ...
  if (fileStat.method == MZ_NO_COMPRESSION) {
    ...
    if (!buffer) {
      LOG("ZIP", "Failed to allocate memory for buffer");
      if (!wasOpen) {
        close();
      }
      return false;
    ...
    while (remaining > 0) {
      ...
      if (dataRead == 0) {
        LOG("ZIP", "Could not read more bytes");
        free(buffer);
        if (!wasOpen) {
          close();
        }
  ...
  if (fileStat.method == MZ_DEFLATED) {
    ...
    if (!fileReadBuffer) {
      LOG("ZIP", "Failed to allocate memory for zip file read buffer");
      free(inflator);
      if (!wasOpen) {
        close();
      }
    ...
    if (!outputBuffer) {
      LOG("ZIP", "Failed to allocate memory for dictionary");
      free(inflator);
      free(fileReadBuffer);
      if (!wasOpen) {
        close();
    ...
    while (true) {
      ...
      if (outBytes > 0) {
        ...
        if (out.write(outputBuffer + outputCursor, outBytes) != outBytes) {
          LOG("ZIP", "Failed to write all output bytes to stream");
          if (!wasOpen) {
            close();
          }
          free(outputBuffer);
      ...
      if (status == TINFL_STATUS_DONE) {
        LOG("ZIP", "Decompressed %d bytes into %d bytes", deflatedDataSize, inflatedDataSize);
        if (!wasOpen) {
          close();
        }
        free(inflator);
