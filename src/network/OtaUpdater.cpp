...
esp_err_t event_handler(esp_http_client_event_t* event) {
  ...
  if (!esp_http_client_is_chunked_response(event->client)) {
    ...
    LOG("OTA", "esp_http_client_is_chunked_response failed, chunked_len: %d", chunked_len);
  }
...
OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  ...
  if (!client_handle) {
    LOG("OTA", "HTTP Client Handle Failed");
    return INTERNAL_UPDATE_ERROR;
  }

  esp_err = esp_http_client_set_header(client_handle, "User-Agent", "CrossPoint-ESP32-" CROSSPOINT_VERSION);
  if (esp_err != ESP_OK) {
    LOG("OTA", "esp_http_client_set_header Failed : %s", esp_err_to_name(esp_err));
  ...
  if (esp_err != ESP_OK) {
    LOG("OTA", "esp_http_client_perform Failed : %s", esp_err_to_name(esp_err));
    esp_http_client_cleanup(client_handle);
    return HTTP_ERROR;
  }
  ...
  if (esp_err != ESP_OK) {
    LOG("OTA", "esp_http_client_cleanupp Failed : %s", esp_err_to_name(esp_err));
    return INTERNAL_UPDATE_ERROR;
  }
  ...
  if (error) {
    LOG("OTA", "JSON parse failed: %s", error.c_str());
    return JSON_PARSE_ERROR;
  }

  if (!doc["tag_name"].is<std::string>()) {
    LOG("OTA", "No tag_name found");
    return JSON_PARSE_ERROR;
  }

  if (!doc["assets"].is<JsonArray>()) {
    LOG("OTA", "No assets found");
    return JSON_PARSE_ERROR;
  }
  ...
  if (!updateAvailable) {
    LOG("OTA", "No firmware.bin asset found");
    return NO_UPDATE;
  }

  LOG("OTA", "Found update: %s", latestVersion.c_str());
  return OK;
}
...
OtaUpdater::OtaUpdaterError OtaUpdater::installUpdate() {
  ...
  if (esp_err != ESP_OK) {
    LOG("OTA", "HTTP OTA Begin Failed: %s", esp_err_to_name(esp_err));
    return INTERNAL_UPDATE_ERROR;
  }
  ...
  if (esp_err != ESP_OK) {
    LOG("OTA", "esp_https_ota_perform Failed: %s", esp_err_to_name(esp_err));
    esp_https_ota_finish(ota_handle);
    return HTTP_ERROR;
  }

  if (!esp_https_ota_is_complete_data_received(ota_handle)) {
    LOG("OTA", "esp_https_ota_is_complete_data_received Failed: %s", esp_err_to_name(esp_err));
    esp_https_ota_finish(ota_handle);
    return INTERNAL_UPDATE_ERROR;
  }
  ...
  if (esp_err != ESP_OK) {
    LOG("OTA", "esp_https_ota_finish Failed: %s", esp_err_to_name(esp_err));
    return INTERNAL_UPDATE_ERROR;
  }

  LOG("OTA", "Update completed");
  return OK;
}
